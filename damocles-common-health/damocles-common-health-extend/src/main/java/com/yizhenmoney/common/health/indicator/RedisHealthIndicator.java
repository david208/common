	...
	protected void doHealthCheck(Health.Builder builder) throws Exception {
		if (valid) {
			...
			if (redisConnectionFactory instanceof JedisConnectionFactory) {
				...
				try {
					builder.up().withDetail(SERVER, jedisConnectionFactory.getClusterConnection().clusterGetNodes().toString());
					builder.withDetail("info", jedisConnectionFactory.getClusterConnection().clusterGetClusterInfo().toString());

					addBuilder(builder);
				} finally {
					RedisConnectionUtils.releaseConnection(jedisConnectionFactory.getClusterConnection(),
